...
class ChannelAdminRights(TLObject):
    ...
    def from_reader(reader):
        ...
        return ChannelAdminRights(change_info=_change_info, post_messages=_post_messages, edit_messages=_edit_messages, delete_messages=_delete_messages, ban_users=_ban_users, invite_users=_invite_users, invite_link=_invite_link, pin_messages=_pin_messages, add_admins=_add_admins, manage_call=_manage_call)
...
class InputMediaUploadedDocument(TLObject):
    ...
    def __init__(self, file, mime_type, attributes, caption, nosound_video=None, thumb=None, stickers=None, ttl_seconds=None):
        """
    ...
    def from_reader(reader):
        ...
        if flags & 2:
            ...
            _ttl_seconds = None
        return InputMediaUploadedDocument(file=_file, mime_type=_mime_type, attributes=_attributes, caption=_caption, nosound_video=_nosound_video, thumb=_thumb, stickers=_stickers, ttl_seconds=_ttl_seconds)
...
class InputSingleMedia(TLObject):
    CONSTRUCTOR_ID = 0x5eaa7809
    SUBCLASS_OF_ID = 0x21ca8ed8

    def __init__(self, media, random_id=None):
        """
    ...
    def from_reader(reader):
        ...
        _random_id = reader.read_long()
        return InputSingleMedia(media=_media, random_id=_random_id)
...
class MessageMediaDocument(TLObject):
    ...
    def __bytes__(self):
        return b''.join((
            b'\xd3\x14D|',
            struct.pack('<I', (0 if self.document is None or self.document is False else 1) | (0 if self.caption is None or self.caption is False else 2) | (0 if self.ttl_seconds is None or self.ttl_seconds is False else 4)),
            b'' if self.document is None or self.document is False else (bytes(self.document)),
            b'' if self.caption is None or self.caption is False else (TLObject.serialize_bytes(self.caption)),
            b'' if self.ttl_seconds is None or self.ttl_seconds is False else (struct.pack('<i', self.ttl_seconds)),
        ))
...
class MessageMediaPhoto(TLObject):
    ...
    def __bytes__(self):
        return b''.join((
            b'\x0f;"\xb5',
            struct.pack('<I', (0 if self.photo is None or self.photo is False else 1) | (0 if self.caption is None or self.caption is False else 2) | (0 if self.ttl_seconds is None or self.ttl_seconds is False else 4)),
            b'' if self.photo is None or self.photo is False else (bytes(self.photo)),
            b'' if self.caption is None or self.caption is False else (TLObject.serialize_bytes(self.caption)),
            b'' if self.ttl_seconds is None or self.ttl_seconds is False else (struct.pack('<i', self.ttl_seconds)),
        ))
